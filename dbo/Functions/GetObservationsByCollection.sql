CREATE FUNCTION [dbo].[GetObservationsByCollection]
(
...
(
	SELECT	vo.observed_feature_namespace,
		...
		FROM [dbo].[V_OBSERVATION] as vo
			INNER JOIN [dbo].[OBSERVATION_COLLECTION] as oc
				ON vo.observation_id = oc.observation_id
			INNER JOIN [dbo].[COLLECTION] c
				ON oc.collection_id = c.collection_id
